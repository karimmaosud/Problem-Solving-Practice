...
public class FreedomTrail {
  ...
  private int solve(
      String key, int ringLength, int cursor, int index, ArrayList<Integer>[] indexes, int[][] dp) {
    if (index == key.length()) {
      return 0;
    }
    ...
    for (int i : indexes[key.charAt(index) - 'a']) {
      ans =
          Math.min(
              ans,
              1
                  + Math.min(Math.abs(i - cursor), ringLength - Math.abs(i - cursor))
                  + solve(key, ringLength, i, index + 1, indexes, dp));
    }
    dp[cursor][index] = ans;
    return ans;
  }
